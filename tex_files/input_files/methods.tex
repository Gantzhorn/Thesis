...
\end{align}
Multiply (\ref{eq:originalOUprocess}) with $\exp\left(\beta t\right)$ and rearrange
\begin{align}
...
\begin{align}
    \mathrm{d}\left(\exp\left(\beta t\right)X_t\right) &= \exp\left(\beta t\right)\beta X_t \mathrm{d}t + \exp\left(\beta t\right) \mathrm{d}X_t \nonumber
\end{align}
...
\begin{align}
    ...
    &X_t = \exp\left(-\beta\left(t - t_0\right)\right)\left(X_{t_0} - \mu\right) + \mu + \int_{t_0}^t \exp\left(-\beta \left(t - s\right)\right)\sigma \mathrm{d}W_s,
\end{align}
which was what we wanted. Now, since the increments of the brownian motion is gaussian, the solution $X_t$ for givne $t$ is gaussian.
