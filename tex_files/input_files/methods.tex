...
\begin{align}
    \mathrm{d}X_t = -\beta\left(X_t - \mu\right)\mathrm{d}t + \sigma \mathrm{d}W_t, \quad X_{t_0} = x_0. \label{eq:originalOUprocess}
\end{align}
In this rather simple process the drift and diffusion are parameterized by $\theta = \left(\beta, \mu, \sigma\right)^\top$ with the constraint that $\beta>0$.
To get the closed form solution for this SDE multiply (\ref{eq:originalOUprocess}) with $\exp\left(\beta t\right)$ and rearrange
\begin{align}
    \exp\left(\beta t\right)\mathrm{d}X_t + \exp\left(\beta t\right) \beta X_t \mathrm{d}t = \exp\left(\beta t\right)\beta\mu \mathrm{d}t + \exp\left(\beta t\right)\sigma \mathrm{d}W_t
\end{align}
...
\begin{align}
    \exp\left(\beta t\right)&X_t - \exp\left(\beta t_0\right)x_0 = \left(\exp\left(\beta t\right) - \exp\left(\beta t_0\right)\right)\mu + \int_{t_0}^t \exp\left(\beta s\right)\sigma \mathrm{d}W_s \nonumber \\
    &X_t = \exp\left(-\beta\left(t - t_0\right)\right)\left(x_0 - \mu\right) + \mu + \int_{t_0}^t \exp\left(-\beta \left(t - s\right)\right)\sigma \mathrm{d}W_s \label{eq:OU_solution},
\end{align}
