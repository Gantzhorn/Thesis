...
\begin{align}
    \mathcal{L}\phi_1(x) = -\beta\left(x - \mu\right)\cdot 1 + \frac{1}{2}\sigma^2 x \cdot 0 = -\beta\left(x - \mu\right) = -\lambda_1\phi_1(x). \label{eq:directVerificationCondMean}
\end{align}
Additionally,
\begin{align}
...
\begin{align}
    ...
    &= - \lambda_2 \varphi_2.
\end{align}
%Since it has the same first eigen function the mean-reverting GBM has the sam one-step ahead conditional mean as (\ref{squarerootCondMean}). In fact, it is not difficult to see that this result generalizes to any pearson diffusion. 
