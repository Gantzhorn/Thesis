...
On the other hand, we can also use the regular Strang-likelihood. The lamperti-transform only depends on the noise term and therefor (\ref{eq:dynamicsquarerootSDE}) has the same lamperti-transform as the one the square-root process has. Let $Y_t:= 2\sqrt{X_t}$ and use Itô's formula on (\ref{eq:dynamicsquarerootSDE}), then
\begin{align}
...
\begin{align}
    ...
    \mathrm{d}Y_t^{[2]} &= - \left(\frac{2}{Y_t}\left(A\left(\frac{Y_t^2}{4} - m\right)^2 + \lambda_t + \frac{\sigma^2}{4}\right) + A_{\mathrm{linear\_part}}\left(Y_t^{[2]} - b_{\mathrm{intercept}}\right)\right)\mathrm{d}t,
\end{align}
where we have use the following constants
\begin{align}
...
We solve these flows using the solution to the Ornstein-Uhlenbeck process and a fourth-order Runge-kutta respectively. The solutions in the usual composition then gives us the Strang-flow, which we maximize in order to get the Strang-based estimator for the lamperti-transformed process.
...
\end{align}
As in the stationary part the lamperti transform is $Y_t:=\log(X_t)$. Therefore, we get from Itô's formula
\begin{align}
...
\begin{align}
    ...
    - A_{\mathrm{linear\_part}}\left(Y_t^{[2]} - \log\left(\zeta\right)\right)\right)\mathrm{d}t.
\end{align}
